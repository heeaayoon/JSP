...
//						+" on M.id = B.id " + " where num=? ";
		...
		String query = "select B.*, M.name from board B left join member M " // INNER를 LEFT로 변경하고 테이블 순서 변경
		...
		try {
			...
			if(rs.next()) { //dto에 값 저장
				...
				dto.setPostdate(rs.getDate("postdate"));
				dto.setId(rs.getString("id"));
				//dto.setVisitcount(rs.getString("visitcount")); //가존
				String visitCountStr = rs.getString("visitcount"); //수정된 내용
		            if (visitCountStr == null) {
		                dto.setVisitcount("0"); // 조회수 데이터가 null이면 "0"으로 설정
		            } else {
		                dto.setVisitcount(visitCountStr); // null이 아니면 원래 값으로 설정
		            }
