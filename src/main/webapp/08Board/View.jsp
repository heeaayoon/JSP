	...
	<form name = "writeFrm">
		...
		<table border = "1" width = "90%">
			<tr>
				...
				<td><%= dto.getNum() %></td>
				<td>작성자</td>
				<%-- %= dto.getName() --%><%-- 기존 : name이 null이면 null로 표시됨 --%>
				<%
    			String writerName = dto.getName();
			    if (writerName == null || writerName.isEmpty()) {
			        writerName = dto.getId(); // name이 없으면 id를 대신 사용
			    }
				%> 
				<td><%= writerName %></td> <!-- name이 있으면 이름이 나오고, 없으면 아이디를 출력 -->
			</tr>
			<tr>
				<td>작성일</td>
